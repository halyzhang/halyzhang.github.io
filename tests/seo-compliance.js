 ...
 */
function testHTMLForSEO(htmlPath, pageName) {
    ...
    try {
        ...
        // Test 13: Check for lang attribute on html tag
        console.log('\nChecking HTML lang attribute...');
        const html = doc.querySelector('html');
        if (html && html.getAttribute('lang')) {
            console.log('✓ HTML lang attribute found');
        } else {
            console.warn('⚠️  HTML lang attribute missing');
            hasWarnings = true;
        }
...
testHTMLForSEO(path.join(projectRoot, '_includes', 'head.html'), 'head.html template');
